...
def predict(params, pred_dirs, out_dir, mask=None, checkpoint='last', redo=False):
    ...
    for pred_dir in pred_dirs:
        ...
        model_name = os.path.basename(params.model_dir)
        name_prefix = os.path.basename(pred_dir.rstrip('/'))
        ...
        if not os.path.isfile(pred_out) or all([os.path.isfile(pred_out), redo]):
            ...
            logging.info("You can overwrite existing data using -x or --overwrite")
...
if __name__ == '__main__':
    ...
    # make predictions
    pred = predict(my_params, study_dirs, args.out_dir, mask=args.mask, checkpoint=args.checkpoint, redo=args.overwrite)
