...
def eval_pred(params, eval_dirs, pred_niis, out_dir, mask, metrics, verbose=False, redo=False, suffix=None):
    ...
    true_niis = [glob(eval_d + '/*' + params.label_prefix[0] + '.nii.gz')[0] for eval_d in eval_dirs]
    pred_niis = sorted(pred_niis)
    if mask:
        mask_niis = [glob(eval_d + '/*' + mask + '.nii.gz') for eval_d in eval_dirs]
        # handle case where eval masks may not exist for all pred files
        bools = [index for index, item in enumerate(mask_niis) if item != []]
        true_niis = [true_niis[x] for x in bools]
        pred_niis = [pred_niis[x] for x in bools]
        mask_niis = [mask_niis[x][0] for x in bools]
    else:
        mask_niis = []
        ...
        for true_nii, pred_nii, mask_nii in zip(true_niis, pred_niis, mask_niis):
            ...
            if not os.path.isfile(crop_true_nii) or redo:
                ...
                if verbose:
                    logging.debug(cmd)
                subprocess.call(cmd, shell=True, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
                # do pixel masking of true nii
                tnii = nib.load(crop_true_nii)
                timg = tnii.get_fdata()
                timg[nib.load(crop_mask_nii).get_fdata() == 0] = 0
                nib.save(nib.Nifti1Image(timg, tnii.affine, tnii.header), crop_true_nii)
            else:
                logging.info(
            ...
            if not os.path.isfile(crop_pred_nii) or redo:
                ...
                if verbose:
                    logging.debug(cmd)
                subprocess.call(cmd, shell=True, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
                # do pixel masking of pred nii
                pnii = nib.load(crop_pred_nii)
                pimg = pnii.get_fdata()
                pimg[nib.load(crop_mask_nii).get_fdata() == 0] = 0
                nib.save(nib.Nifti1Image(pimg, pnii.affine, pnii.header), crop_pred_nii)
            else:
                logging.info(
